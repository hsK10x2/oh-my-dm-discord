...
- Discord 서버 채널 조회와 전송 — 실행 후 서버를 자동으로 하나씩 불러옵니다
- 대화 목록을 다이렉트 메시지와 서버별 섹션으로 구분해 표시
- 안 읽은 대화 표시
...
| Discord | macOS, Linux, Windows | 번들된 Playwright Chromium으로 DM과 서버 텍스트 채널을 읽고 씁니다. DM이 먼저 뜨고, 서버는 백그라운드에서 하나씩 자동으로 추가됩니다(폴더에 접힌 서버도 자동으로 펼칩니다). 대화 목록은 다이렉트 메시지와 서버별 섹션으로 나뉩니다. 음성 채널과 스레드는 다루지 않습니다. |
...
- Browse and send Discord server channels — servers load on their own after launch
- Group the conversation list into direct messages and one section per server
- Show which conversations are unread
...
| Discord | macOS, Linux, Windows | Reads and writes both DMs and server text channels through the bundled Playwright Chromium. DMs appear first and servers are added one at a time in the background, expanding collapsed server folders on the way. The conversation list is grouped into direct messages and one section per server. Voice channels and threads are out of scope. |
